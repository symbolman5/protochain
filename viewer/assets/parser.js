...
var ProtochainParser = (() => {
  ...
  // viewer/parser-entry.ts
  var parser_entry_exports = {};
  __export(parser_entry_exports, {
    PARSER_VERSION: () => PARSER_VERSION,
    parseProtocolContent: () => parseProtocolContent
  });
...
${end.comment}` : end.comment;
  ...
  var ParseError = class extends Error {
    constructor(message, section, line) {
      super(message);
      __publicField(this, "section", section);
  ...
  function parseMarkdownLightweight(markdown) {
    ...
    while (i < lines.length) {
      ...
      const paraLines = [];
      while (i < lines.length && lines[i].trim() !== "" && !lines[i].trim().startsWith("```") && !/^#{1,6}\s+/.test(lines[i]) && !/^\s*[-*+]\s+/.test(lines[i]) && !/^\s*\d+\.\s+/.test(lines[i]) && !(lines[i].includes("|") && i + 1 < lines.length && /^\s*\|?[\s:-]+\|/.test(lines[i + 1]))) {
  ...
  function findFirstTable(nodes) {
    for (const node of nodes) {
      if (node.type === "table") return node;
    }
    return null;
  }
  function findFirstCodeBlock(nodes, lang) {
    for (const node of nodes) {
      if (node.type === "code") {
        const code = node;
  ...
  function tableToObjects(table, headerNormalizer) {
  ...
  // src/parser/extension-sections.ts
  function matchSectionByKeywords(section, keywords) {
  ...
  function detectExtensionSection(sections, keywords, sectionName) {
  ...
  function detectExtensionSectionList(sections, keywords, sectionName, itemParser) {
  ...
  function detectExtensionSectionObject(sections, keywords, sectionName, objectParser) {
    const detection = detectExtensionSection(sections, keywords, sectionName);
    if (!detection.enabled) return void 0;
    return objectParser(detection.yaml, detection.section);
  }

  // src/parser/index.ts
  function parseProtocolContent(content, sourcePath, options = {}) {
  ...
  function parseRoles(raw) {
    ...
    return raw.map((item, idx) => {
      ...
      const role = {
        ...
        anonymous: typeof r.anonymous === "boolean" ? r.anonymous : void 0
      };
      if (role.anonymous === void 0) delete role.anonymous;
      return role;
    });
  }
  function parseRoleType(v) {
    if (v === void 0 || v === null) return "participant";
    if (v === "consensus" || v === "participant") return v;
    throw new ParseError(`roleType \u5FC5\u987B\u662F consensus \u6216 participant\uFF0C\u5B9E\u9645\u4E3A ${v}`);
  }
  function parseChangeDeclarations(raw) {
  ...
  var READABLE_SECTIONS = /* @__PURE__ */ new Map([
    ["\u80CC\u666F", "background"],
    ["\u80CC\u666F\u4E0E\u76EE\u6807", "background"],
    ["\u534F\u8BAE\u80CC\u666F", "background"],
    ["\u6838\u5FC3\u6982\u5FF5", "concepts"],
  ...
  function parseReadableLayer(sections) {
    const result = {
      background: "",
      concepts: [],
      workflow: ""
  ...
  function collectParagraphText(nodes) {
    return nodes.filter((n) => n.type === "paragraph" || n.type === "list").map((n) => nodeToText(n)).join("\n\n").trim();
  }
  var DERIVABLE_TABLE_SECTIONS = /* @__PURE__ */ new Map([
    ["\u72B6\u6001\u7A7A\u95F4", "states"],
    ["\u72B6\u6001", "states"],
    ["\u8F6C\u79FB\u89C4\u5219", "transitions"],
    ["\u8F6C\u79FB", "transitions"],
  ...
  var DERIVABLE_FORMAL_SECTIONS = /* @__PURE__ */ new Set([
    "\u53EF\u63A8\u6F14\u5C42",
    "\u53EF\u63A8\u6F14\u5C42(\u5F62\u5F0F\u5316)",
    "\u53EF\u63A8\u6F14\u5C42\uFF08\u5F62\u5F0F\u5316\uFF09",
    "\u5F62\u5F0F\u5316\u89C4\u683C",
  ...
  function parseDerivableLayer(sections, metadata, allowDegraded) {
    ...
    const derivable = {
      ...
      initialStateId,
      terminalStateIds,
      resourcePools: resourcePools.length > 0 ? resourcePools : void 0,
      instantiation,
      externalEvents: externalEvents.length > 0 ? externalEvents : void 0,
      negativeAssurances: negativeAssurances.length > 0 ? negativeAssurances : void 0,
      subsidiaryEntities: subsidiaryEntities.length > 0 ? subsidiaryEntities : void 0,
      guardTranslations: guardTranslations.length > 0 ? guardTranslations : void 0
    };
    const contractInput = parseContractInput(sections);
    return { derivable, contractInput };
  }
  function findFormalCodeBlock(sections) {
    for (const section of sections) {
  ...
  function matchDerivableTableSection(heading) {
  ...
  function rowToState(row) {
    ...
    const type = typeRaw === "initial" || typeRaw === "\u521D\u59CB" ? "initial" : typeRaw === "terminal" || typeRaw === "\u7EC8\u6001" ? "terminal" : typeRaw === "error" || typeRaw === "\u9519\u8BEF" ? "error" : "normal";
    const roles = row.roles ? row.roles.split(/[|,，]/).map((s) => s.trim()).filter(Boolean) : void 0;
    return {
      id,
      name,
      type,
      description: row.description || void 0,
      roleIds: roles && roles.length > 0 ? roles : void 0
  ...
  function rowToTransition(row) {
    ...
    const effects = row.effects ? row.effects.split(/[|;；]/).map((s) => s.trim()).filter(Boolean) : void 0;
    const triggerTypeRaw = (row.triggertype || "").toLowerCase();
    const triggerType = triggerTypeRaw === "role" || triggerTypeRaw === "system" || triggerTypeRaw === "external" ? triggerTypeRaw : void 0;
    const affectsDimensions = row.affectsdimensions ? row.affectsdimensions.split(/[|,，]/).map((s) => s.trim()).filter(Boolean) : void 0;
    const actionTypeRaw = (row.actiontype || "").toLowerCase();
    const actionType = actionTypeRaw === "state_transition" || actionTypeRaw === "attribute_update" ? actionTypeRaw : void 0;
    const t = {
      id,
      name,
      from: fromList,
      to,
      action,
      triggerRoleId: row.trigger || void 0,
      guard: row.guard || void 0,
      effects: effects && effects.length > 0 ? effects : void 0,
      isException: false,
  ...
  function rowToInvariant(row) {
    ...
    const scope = row.scope ? row.scope.split(/[|,，]/).map((s) => s.trim()).filter(Boolean) : void 0;
    const invariantClassRaw = (row.invariantclass || "").toLowerCase();
    const invariantClass = invariantClassRaw === "intra_protocol" || invariantClassRaw === "cross_protocol" || invariantClassRaw === "cross_instance" ? invariantClassRaw : void 0;
    const levelRaw = (row.level || "").toLowerCase();
    const level = levelRaw === "state-machine" || levelRaw === "data" || levelRaw === "\u72B6\u6001\u673A" || levelRaw === "\u6570\u636E" ? levelRaw === "\u72B6\u6001\u673A" ? "state-machine" : levelRaw === "\u6570\u636E" ? "data" : levelRaw : void 0;
    const sourceRaw = (row.source || "").toLowerCase();
    const source = sourceRaw === "storage" || sourceRaw === "guard" || sourceRaw === "\u5B58\u50A8" || sourceRaw === "\u5B88\u536B" ? sourceRaw === "\u5B58\u50A8" ? "storage" : sourceRaw === "\u5B88\u536B" ? "guard" : sourceRaw : void 0;
    const inv = {
      id,
      name,
      expression,
      scopeStateIds: scope && scope.length > 0 ? scope : void 0,
      description: row.description || void 0,
  ...
  function rowToTiming(row) {
    ...
    const type = typeRaw === "deadline" || typeRaw === "\u622A\u6B62" ? "deadline" : typeRaw === "timeout" || typeRaw === "\u8D85\u65F6" ? "timeout" : typeRaw === "ordering" || typeRaw === "\u987A\u5E8F" ? "ordering" : typeRaw === "continuous" || typeRaw === "\u6301\u7EED" ? "continuous" : typeRaw === "scheduled" || typeRaw === "\u5B9A\u65F6" ? "scheduled" : "response";
    const boundMs = row.boundms ? parseInt(row.boundms, 10) : void 0;
    const t = {
      id,
      name,
      type,
      ...
      boundMs: Number.isFinite(boundMs) ? boundMs : void 0,
      onViolation: row.onviolation || void 0,
  ...
  function rowToException(row) {
    ...
    const transitionIds = (row.transitionids || "").split(/[|,，]/).map((s) => s.trim()).filter(Boolean);
    const errorCode = row.errorcode ? row.errorcode.trim() : void 0;
    return {
      id,
      name,
      trigger,
      transitionIds,
      recovery: row.recovery || void 0,
      errorCode: errorCode && errorCode.length > 0 ? errorCode : void 0
    };
  }
  function applyLegacyMigration(states, transitions, invariants, _timing, metadata) {
  ...
  function parseResourcePools(sections) {
  ...
  function parseResourcePoolItem(yaml) {
    ...
    return {
      ...
      crossInvariantIds: asStringArray(r.crossInvariantIds, "\u8D44\u6E90\u6C60.crossInvariantIds") ?? void 0
    };
  }
  function parseInstantiation(sections) {
  ...
  function parseExternalEvents(sections) {
    return detectExtensionSectionList(
      sections,
      ["\u5916\u90E8\u4E8B\u4EF6", "externalevent"],
      "\u5916\u90E8\u4E8B\u4EF6",
  ...
  function parseNegativeAssurances(sections, metadata) {
  ...
  function parseSubsidiaryEntities(sections) {
    return detectExtensionSectionList(
      sections,
  ...
  function parseGuardTranslations(sections) {
    return detectExtensionSectionList(
      sections,
  ...
  function parseDimensions(yaml, fieldPath) {
    if (!yaml) return [];
    if (!Array.isArray(yaml)) {
  ...
  function parseContractInput(sections) {
    const contractSection = sections.find(
      (s) => s.heading.includes("\u5951\u7EA6\u5C42") || s.heading.includes("\u5951\u7EA6")
    );
    if (!contractSection) return void 0;
    ...
    if (parsedYaml) {
      const result = {
        parties: Array.isArray(parsedYaml.parties) ? parsedYaml.parties : [],
        expectedInformationFields: Array.isArray(parsedYaml.expectedInformationFields) ? parsedYaml.expectedInformationFields : void 0
      };
      const contractsRaw = parsedYaml.contracts;
      if (Array.isArray(contractsRaw)) {
        result.contracts = contractsRaw.map(
          (c, idx) => parseContractEntry(c, `contracts[${idx}]`)
        );
      }
      return result;
    }
    ...
    if (parties.length === 0) return void 0;
    return { parties };
  }
  function parseContractEntry(raw, path) {
  ...
  function parseErrorResponses(raw, path) {
    if (!Array.isArray(raw)) {
    ...
    return raw.map((item, idx) => {
      ...
      if (r.bodySchema !== void 0 && r.bodySchema !== null) {
  ...
  function parseJsonSchemaValue(raw, path) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
      throw new ParseError(`${path} \u5FC5\u987B\u662F\u5BF9\u8C61`);
    }
    const r = raw;
    ...
    if (r.type !== void 0) {
      const t = r.type;
      if (t !== "string" && t !== "number" && t !== "integer" && t !== "boolean" && t !== "object" && t !== "array" && t !== "null") {
    ...
    if (r.properties !== void 0) {
      ...
      schema4.properties = props;
    }
    if (r.required !== void 0) {
      if (!Array.isArray(r.required)) {
        throw new ParseError(`${path}.required \u5FC5\u987B\u662F\u5B57\u7B26\u4E32\u6570\u7EC4`);
      }
      schema4.required = r.required.map((x, i) => {
        if (typeof x !== "string") {
    ...
    if (r.items !== void 0) {
      schema4.items = parseJsonSchemaValue(r.items, `${path}.items`);
    }
    if (r.enum !== void 0) {
    ...
    if (typeof r.description === "string") schema4.description = r.description;
    if (typeof r.format === "string") schema4.format = r.format;
    if (r.default !== void 0) schema4.default = r.default;
    if (typeof r.minItems === "number") schema4.minItems = r.minItems;
    if (typeof r.maxItems === "number") schema4.maxItems = r.maxItems;
    if (r.additionalProperties !== void 0) {
      if (typeof r.additionalProperties === "boolean") {
        schema4.additionalProperties = r.additionalProperties;
      } else if (r.additionalProperties && typeof r.additionalProperties === "object") {
        schema4.additionalProperties = parseJsonSchemaValue(
    ...
    if (schema4.type === void 0 && (schema4.properties !== void 0 || schema4.required !== void 0)) {
      schema4.type = "object";
    }
    return schema4;
  }
  function parseExpressionArray(raw, path) {
    if (!Array.isArray(raw)) {
      throw new ParseError(`${path} \u5FC5\u987B\u662F\u6570\u7EC4`);
    }
    return raw.map((item, idx) => {
      ...
      if (item && typeof item === "object" && !Array.isArray(item)) {
        ...
        if (k === "json-schema") {
          return {
            kind: "json-schema",
            description: typeof expr.description === "string" ? expr.description : void 0,
            schema: expr.schema !== void 0 ? parseJsonSchemaValue(expr.schema, `${path}[${idx}].schema`) : void 0
          };
        }
  ...
  function requireString(obj, key, section) {
    const v = obj[key];
  ...
  function requireRowField(row, key, section) {
    const v = row[key];
    if (!v || v.trim() === "") {
  ...
  function asRecord(yaml, section) {
    if (!yaml || typeof yaml !== "object" || Array.isArray(yaml)) {
      throw new ParseError(`${section} \u7684 YAML \u5FC5\u987B\u662F\u5BF9\u8C61`);
    }
    return yaml;
  }
  function asStringArray(yaml, fieldPath) {
    if (yaml === void 0 || yaml === null) return [];
    if (!Array.isArray(yaml)) {
  ...
  function asRecordArray(yaml, fieldPath) {
    if (yaml === void 0 || yaml === null) return [];
    if (!Array.isArray(yaml)) {
  ...
  // viewer/parser-entry.ts
  var PARSER_VERSION = true ? "0.1.0" : "unknown";
