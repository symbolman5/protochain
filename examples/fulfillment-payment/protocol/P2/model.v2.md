...
name: 支付协议
version: 1.1.0
purpose: 支付协议 v1→v2 演进素材（TC6 ⑤ / TC9 ⑦ diff 载体）：新增"部分退款"转移 T5 + metadata.version 升级
roles:
  - id: customer
    name: 顾客
...
支付协议定义顾客发起支付、支付网关回调结果、平台审批退款的协作规则。v2 新增部分退款能力：已支付订单可在全额退款之外进行部分退款（T5）。核心约束不变：支付金额必须与订单金额一致（`paid_amount == order_amount`），支付幂等键全局唯一（`unique(payment_id)`）。

# 核心概念

- **支付幂等键**：payment_id 全局唯一，支付网关事件重复推送时幂等处理
- **支付回调**：支付网关在支付结果确定后推送回调，触发状态迁移
- **退款**：仅已支付订单可发起退款；v2 支持部分退款（退款金额 ≤ 已支付金额）

# 协作流程

顾客发起支付后订单进入支付中状态，支付网关回调成功则进入已支付状态；平台审批通过后发起全额退款（T4）或部分退款（T5），订单进入已退款终态。
...
| T5 | 部分退款 | S2 | S3 | refund_partial | platform | partial_refund_approved | | role | state_transition | |
...
contracts:
  ...
  - interface: refund_partial
    requestSchema:
      type: object
      properties:
        partial_amount: { type: number }
        refund_reason: { type: string }
      required: [partial_amount, refund_reason]
```
