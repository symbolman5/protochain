...
> [← 返回 P1 履约协议](../P1/) | 接口 ID: `IF_SYS_T3` | 类型: **系统** | schemaKind: **structured**

## 输入字段

| 字段名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| fulfillment_qty | integer | ✓ | complete_fulfillment 契约层请求字段（字段 fulfillment_qty，类型 integer） |
| order_qty | integer | ✓ | complete_fulfillment 契约层请求字段（字段 order_qty，类型 integer） |
