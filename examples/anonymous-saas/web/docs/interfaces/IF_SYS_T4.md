...
| effects | array | 副作用：处置状态=已移除; 访问策略=拒绝（底层实例随后回收，INV-10） |  |

## 前置条件 (preconditions)

- kind=`legacy-stub`：guard 表达式含中文标点，自然语言未机械提取：处置状态≠已移除

## 后置条件 (postconditionExpressions)

- 处置状态=已移除
- 访问策略=拒绝（底层实例随后回收，INV-10）
