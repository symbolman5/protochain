...
> 接口 ID: `IF_SYS_T22` | 类型: **系统** | 动作类型: `state_transition` | schemaKind: **legacy-stub**

## 降级理由

- guard 表达式 "周期性巡检" 未机械提取为 JSON Schema（未按受限谓词语法书写，显式降级不静默，R2-1）
...
| effects | array | 副作用：有效期档=有效或临近过期或已过期 |  |

## 前置条件 (preconditions)

- kind=`legacy-stub`：guard 表达式含中文标点，自然语言未机械提取：周期性巡检
